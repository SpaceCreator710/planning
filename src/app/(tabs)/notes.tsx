...
export default function NotesTab() {
  ...
  function save() {
    if (!title.trim() && !body.trim()) {
      Alert.alert('Write something first', 'Add a title or note text before saving.');
      return;
    }
    ...
    setTitle('');
    setBody('');
    Alert.alert(editingId ? 'Changes saved' : 'Note saved');
  }
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: spacing.md, paddingBottom: 120, gap: spacing.xl }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
        ...
        <GlassIconButton icon="square.and.pencil" accessibilityLabel="Create note" active onPress={newNote} />
      </View>
      ...
      <Card muted style={{ gap: spacing.md }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
          ...
          <View style={{ flex: 1 }}>
          <AppText variant="heading">{process.env.EXPO_OS === 'web' ? 'Built-in Notes' : 'Apple Notes bridge'}</AppText>
            <AppText variant="small" tone="secondary">{process.env.EXPO_OS === 'web' ? 'Create, edit, search and turn notes into plan inputs here. Apple’s Share extension becomes available in the installed iPhone app.' : 'In Apple Notes, tap Share, then choose Plan Your Day. To send a note back, use “Share” below and choose Notes.'}</AppText>
          </View>
        </View>
        <AppText variant="caption" tone="tertiary">{process.env.EXPO_OS === 'web' ? 'Safari cannot read the Apple Notes database. Your notes on this screen still work and stay saved locally.' : 'Apple does not provide third-party apps full database access to Apple Notes, so the system Share Sheet is the private, official handoff.'}</AppText>
      </Card>
      ...
      <View style={{ gap: spacing.sm }}>
        {notes.map((note) => (
            <Card key={note.id} style={{ gap: spacing.sm }}>
              ...
              <View style={{ flexDirection: 'row', gap: spacing.sm }}>
                <AppButton compact title="Edit" variant="secondary" onPress={() => edit(note.id)} style={{ flex: 1 }} />
        ...
        {!notes.length ? (
          <View style={{ minHeight: 220, borderRadius: radii.xl, borderCurve: 'continuous', backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center', gap: spacing.sm }}>
